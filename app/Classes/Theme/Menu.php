...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            if(isset($item['title']) == 'Documentation' && isset($item['page']) && $item['page'] == 'dlink_admin' && config('constants.APP_ENV')!='production'){
                 $item_class .= ' dnone'; 
            }
            ...
            if (isset($item['parent'])) {
                ...
                if (isset($item['page'])) {
                    if($item['page'] == 'dlink'){
                        $url = config('constants.DOC_LINK');
                    }
                    elseif($item['page'] == 'dlink_admin'){
                        $url = config('constants.SUPER_ADMIN_DOC_LINK');
                    }else{
                        $url = url($item['page']);
                    }
                }
